...
function Calendar({
  ...
  return (
    <DayPicker
      dir="rtl" // Set direction to RTL for DayPicker
      showOutsideDays={showOutsideDays}
      className={cn("p-3", className)}
      classNames={{
        months: "flex flex-col sm:flex-row space-y-4 sm:space-x-reverse sm:space-x-4 sm:space-y-0", // Use space-x-reverse for RTL
        month: "space-y-4",
        caption: "flex justify-center pt-1 relative items-center",
        caption_label: "text-sm font-medium",
        nav: "space-x-reverse space-x-1 flex items-center", // Use space-x-reverse for RTL
        nav_button: cn(
          buttonVariants({ variant: "outline" }),
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100"
        ),
        // Adjust nav button positions for RTL
